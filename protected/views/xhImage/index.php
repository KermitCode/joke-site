...
<div class="stat">
    <div class="stats">
        <span class="left"><a href="<?php echo $this->createUrl('site/index');?>">首页</a> -> <a href="<?php echo $this->createUrl('XhImage/index');?>">图片笑话列表</a> <?php
        if($id) echo "-><a href='#'>{$this->xh_class[$id]['name']}</a>";?>
        </span>
...
<div class="photos">
	<div class="mat wid9"><div class="left">图片笑话所有分类</DIV><div class="right more"></div></div>
    <div class="imgcla">
	<?php foreach($this->xh_class as $key=>$row){ if($row['type']==2) echo "<a href='".$this->createUrl('XhImage/index',array('id'=>$key))."'>{$row['name']}({$row['nums']})</a>";} ?>
    </div>                
</div>

<div class="photos">
	<div class="mat wid9"><div class="left"><?php if($id) echo $this->xh_class[$id]['name'];else echo '所有';?>图片笑话列表</DIV><div class="right more"></div></div>
    <div class="phlistall">
	  <?php 
...
<div class="main wid308 left mart10<?php if($i!=0) echo ' marl20';?>">             
    ...
    <div class="list wid310"><ul><?php 
          foreach($this->getClassrs($classid) as $row){
              echo "<li><span class='left'><a href='".$this->createUrl('XhTitle/view',array('id'=>$row['tt_id']))."' target='_blank'>".$this->kermitBase->str_cut($row['tt_title'],30)."</a></span>";#<span class='views'>({$row['tt_showviews']})</span>
